...
<div class="modules">
    ...
    if (isset($_GET[install]))
    {
        ...
        if ($freeSize < 70 || $freeSize < $size + 5)
        {
            ...
            if (exec("mount | grep \"on /usb\" -c") >= 1)
                $warning .= "<a href=\"index.php?modules&doInstall=usb&name=$name&version=$version&md5=$md5\">" . $strings["modules-usbInstall"] . "</a>";
        }
        ...
        else
        {
            ...
            if (exec("mount | grep \"on /usb\" -c") >= 1)
                echo "<a href=\"index.php?modules&doInstall=usb&name=$name&version=$version&md5=$md5\">" . $strings["modules-install-external"] . "</a>";
        }
        echo "</div><br /><br />";
    }
    ...
    <div class="moduleContent" align=left>
        <?php
        if (!isset($_GET[show]))
            echo "<a href=\"index.php?modules&show\">" . $strings["modules-available-list"] . "</a><br />" . $strings["modules-available-warning"];
        else
        {
            $remoteModules = getRemoteList();
            if (trim($remoteModules[0]) == "")
                echo "<center><font color=red>" . $strings["modules-available-error"] . "</font></center>";
            else
                drawRemoteModules($remoteModules);
        }
        ?>
    </div>
...
{
    if ($dest == "internal")
        $link = "<b><a href='/infusions/" . $name . "/" . $startPage . "'><font color=black>$name</font></a></b>";
    else
        $link = "<b><a href='/infusions/usbinfusions/" . $name . "/" . $startPage . "'><font color=black>$name</font></a></b>";
    $links = explode("\n", file_get_contents("/pineapple/includes/moduleNav"));
    if (exec("cat /pineapple/includes/moduleNav | grep '$link'") != "")
        return true;
    return false;
}
...
function removeModule($name, $version, $dest)
{
    ...
    if ($dest == "internal")
        exec("rm -rf /pineapple/infusions/" . $name);
    else
        exec("rm -rf /pineapple/infusions/usbinfusions/" . $name);
    unpinFromNav($name);
}
...
function isInstalled($module)
{
    ...
    foreach ($localModules as $localModule)
    {
        ...
        if ($localModule[0] == $module[0])
        {
            ...
            {
                if ($updateWarning != true)
                    echo "<script type='text/javascript'>alert('" . $strings["modules-js-updateAlert"] . "');</script>";
                $updateWarning = true;
                return 2;
            }
            else
...
function downloadLink($module)
{
    ...
    else if ($status == 1)
    {
        ...
        return"<a href=\"index.php?modules&install&name=$module[0]&version=$module[1]&size=$module[4]&md5=$module[5]\">" . $strings["modules-links-install"] . "</a>";
}
