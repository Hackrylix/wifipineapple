...
{
    exec("cp /etc/conf/backup/* /etc/conf/backup/");
...
<div class=content>
    ...
    <div class=contentContent>
        <form action='' method= 'post' ><textarea rows="10" name="zcommand" style='min-width:100%; font-family:courier; font-weight:bold; background-color:black; color:white; border-style:dashed;'></textarea>
            <center>
                <input type='submit' value='<?= $strings["advanced-execute"] ?>'> <small><font color="black"><br />Will execute one command per line</font></small>
            </center>
        </form>
        <?php
    ...
    <div class=contentContent>
        ...
        <form action='index.php?advanced' method= 'post' >
            ...
            <input type='submit' value='<?= $strings["advanced-routing-button"] ?>'> <small><?= $strings["advanced-routing-example"] ?> <i>route add default gw 172.16.42.42 br-lan</i> <br /></small>
        </form>
        <br />
        <?php
    ...
    <div class=contentContent>
        ...
        if (isset($_POST['changePass']))
        {
            if ($_POST['pass'] == $_POST['pass2'] && $_POST['pass'] != "")
            {
                ...
                $passMessage = "<font color=red>" . $strings["advanced-password-unsuccessful"] . "</font><br /><br />";
        }
        ?>
